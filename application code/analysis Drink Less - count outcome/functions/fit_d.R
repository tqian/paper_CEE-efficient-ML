...
train_d_part2 <- function(dta_train,
  ...
  if (d_model_type == "linear-over-t") {
    ...
    fit_formula <- as.formula(paste0("phi_otimes2~", paste0(moderator, collapse = "+"), "+t-1"))
    ...
    # }
      # print(sum(dta_train[, moderator] != rep(1:total_T, times = sample_size)))
    
    # tune parameter
    # if (tune_param) {
    #   
    #   fit <- train(fit_formula, method = "earth", data = pred_df,
    #                tuneGrid = tune_grid, metric = tune_metric,
    #                trControl = trainControl(method = "cv", number = tune_cv_fold))
    #   
    # } else {
    ...
    pred_df <- as.data.frame(predictor)
    colnames(pred_df) <- c(moderator, "t", "phi_otimes2")
    # fit_formula <- as.formula(paste0("phi_otimes2~", paste0(moderator, collapse = "+"), "+s(t)"))
    if (is.null(moderator) | sum(dta_train[, moderator] != rep(1:total_T, times = sample_size)) == 0) {
    ...
    return(fit)
  } else if (d_model_type == "stack") {
    
    stack.model <- c("gam", "earth", "ranger")
    predictor <- cbind(dta_train[, moderator], rep(1:total_T, times = sample_size), phi_otimes2)
    pred_df <- as.data.frame(predictor)
    colnames(pred_df) <- c(moderator, "t", "phi_otimes2")
    
    control <- c(moderator, "t")
    outcome <- "phi_otimes2"
    ...
    stack.fit <-fits$stack.fit.a0
    
    # get predictions from weak learners
    
    # A = 1
    pred_learners <- get_predictions(stack.fit, pred_df, "phi_otimes_hat")
    
    # get weights
    weights <- stack.lm(pred_learners, pred_df, outcome, family = gaussian)
    
    return(list(weights = weights, stack.fit = stack.fit))
...
fit_d_part_all <- function(dta_holdout,
  ...
  if (d_model_type %in% c("linear-over-t", "earth", "gam")) {
    ...
    pred_learners <- get_predictions(fit_train$stack.fit, predictor_holdout, "phi_otimes_hat")
    
    pred_values_holdout <- as.matrix(pred_learners) %*% fit_train$weights
  ...
  d_part <- sapply(as.vector(pred_values_holdout), function(x){MASS::ginv(x)})
  } else {
...
train_d_part1 <- function(
  ...
  if (outcome_type == "continuous") {
    ...
    if (d_model_type == "linear-over-t") {
      ...
      for (t in 1:total_T) {
        it_idx <-  seq(from = t, by = total_T, length = sample_size)
        t_specific_estimate[t, ] <- mean(phi[it_idx])
      }
      ...
      colnames(pred_df) <- c(moderator, "t", "phi")
      # if (is.null(moderator) | sum(dta_train[, moderator] != rep(1:total_T, times = sample_size)) == 0) {
      #   fit_formula <- as.formula(paste0("phi~", "t"))
      # } else {
      fit_formula <- as.formula(paste0("phi~", paste0(moderator, collapse = "+"), "+t"))
      # }
      # print(fit_formula)
      ...
      fit_formula <- as.formula(paste0("phi~", paste0(moderator, collapse = "+"), "+s(t)"))
      }
      ...
      # A = 1
      pred_learners <- get_predictions(stack.fit, pred_df, "phi_hat")
