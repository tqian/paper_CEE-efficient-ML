...
check_unique_values <- function(dta, control, nunique = 5) {
  
  cts_control <- discrete_control <- c()
  
  for (icontrol in 1:length(control)) {
    # print(unique(dta[[control[icontrol]]]))
    if (length(unique(dta[[control[icontrol]]])) > nunique) {
      cts_control <- c(cts_control, control[icontrol])
    } else {
      discrete_control <- c(discrete_control, control[icontrol])
    }
  }
  
  return(list(cts_control = cts_control, discrete_control = discrete_control))
}
...
get_stack.model <- function(models, dta_a0, dta_a1, control, outcome, gam_control_spline_var) {
  ...
  for (i in 1:length(models)) {
    ...
    if (ml_method == "mean") {
      ...
      ## separate continuous and categorical variables based on unique values > 5
      # separate_control_a0 <- check_unique_values(dta_a0, control)
      # separate_control_a1 <- check_unique_values(dta_a1, control)
      # cts_control_a0 <- separate_control_a0$cts_control
      # discrete_control_a0 <- separate_control_a0$discrete_control
      # cts_control_a1 <- separate_control_a1$cts_control
      # discrete_control_a1 <- separate_control_a1$discrete_control
      # if (length(discrete_control_a0) == 0) {
      #   gam_formula_a0 <- as.formula(paste0(outcome, " ~ ", paste0("s(", cts_control_a0, ")", collapse = " + ")))
      # } else {
      # gam_formula_a0 <- as.formula(paste0(outcome, " ~ ", paste0("s(", cts_control_a0, ")", collapse = " + "), "+",
      #                                     paste0(discrete_control_a0, collapse = " + ")))
      # }
      # 
      # if (length(discrete_control_a1) == 0) {
      #   gam_formula_a1 <- as.formula(paste0(outcome, " ~ ", paste0("s(", cts_control_a1, ")", collapse = " + ")))
      # } else {
      # gam_formula_a1 <- as.formula(paste0(outcome, " ~ ", paste0("s(", cts_control_a1, ")", collapse = " + "), "+",
      #                                     paste0(discrete_control_a1, collapse = " + ")))
      # }
      # print(gam_formula_a0)
...
fit_mu_cf <- function(
  ...
  if (ml_method == "sl.smooth") {
    ...
    stack.model <- c("gam", "ranger", "glm", "earth")
  }
