...
design_bin <- expand.grid(moderator = c("NULL", "decision_index"),
                           outcome = c("binary_8to9"),
                           stringsAsFactors = FALSE)

set.seed(1)

for (idesign in 1:nrow(design_bin)) {
  
  print(paste0("idesign = ", idesign))
  
  outcome <- design_bin$outcome[idesign]
  moderator <- design_bin$moderator[idesign]
  model_for_var <- design_bin$model_for_var[idesign]
  ...
  filename <- paste0("bin ", idesign, " - outcome = ", outcome, ", moderator = ", moderator)
