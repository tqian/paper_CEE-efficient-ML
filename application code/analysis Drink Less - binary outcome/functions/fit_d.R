...
train_d_part2 <- function(dta_train,
  ...
  # mu_hat_a0 <- ifelse(mu_hat_a0 == 0, 0.1, mu_hat_a0)
  # print(mu_hat_a0)
  # print(mu_hat_a1)
  ...
  if (d_model_type == "linear-over-t") {
  ...
  } else if (d_model_type == "exp-over-t") {
      predictor <- cbind(dta_train[, moderator], exp(rep(1:total_T, times = sample_size)), phi_otimes2)
      pred_df <- as.data.frame(predictor)
      colnames(pred_df) <- c(moderator, "t", "phi_otimes2")
      fit_formula <- as.formula(paste0("phi_otimes2~", paste0(moderator, collapse = "+"), "+t"))
      fit <- lm(fit_formula, data = pred_df)
    ...
    # print(summary(fit))
    return(fit)
  } else if (d_model_type == "earth-exp") {
    predictor <- cbind(dta_train[, moderator], exp(rep(1:total_T, times = sample_size)), phi_otimes2)
    pred_df <- as.data.frame(predictor)
    colnames(pred_df) <- c(moderator, "t", "phi_otimes2")
    fit_formula <- as.formula(paste0("phi_otimes2~", paste0(moderator, collapse = "+"), "+t"))
    fit <- earth::earth(fit_formula, data = pred_df)
    return(fit)
    ...
    return(fit)
  } else if (d_model_type == "earth-exp-linear") {
    predictor <- cbind(dta_train[, moderator], exp(rep(1:total_T, times = sample_size)), rep(1:total_T, times = sample_size), phi_otimes2)
    pred_df <- as.data.frame(predictor)
    colnames(pred_df) <- c(moderator, "exp_t", "t", "phi_otimes2")
    fit_formula <- as.formula(paste0("phi_otimes2~", paste0(moderator, collapse = "+"), "+exp_t+t"))
    fit <- earth::earth(fit_formula, data = pred_df)
    return(fit)
...
fit_d_part_all <- function(dta_holdout,
  ...
  if (d_model_type %in% c("linear-over-t", "earth", "rf", "gam")) {
    # predict on holdout set for linear-over-t model or earth model
  ...
  } else if (d_model_type %in% c("exp-over-t", "earth-exp")) {
    predictor_holdout <- cbind(dta_holdout[, moderator], exp(rep(1:total_T, times = sample_size_holdout)))
    predictor_holdout <- as.data.frame(predictor_holdout)
    colnames(predictor_holdout) <- c(moderator, "t")
    pred_values <- predict(fit_train, newdata = predictor_holdout)
  ...
  } else if (d_model_type %in% c("earth-exp-linear")) {
    predictor_holdout <- cbind(dta_holdout[, moderator], exp(rep(1:total_T, times = sample_size_holdout)), rep(1:total_T, times = sample_size_holdout))
    predictor_holdout <- as.data.frame(predictor_holdout)
    colnames(predictor_holdout) <- c(moderator, "exp_t", "t")
    pred_values <- predict(fit_train, newdata = predictor_holdout)
...
train_d_part1 <- function(
  ...
  if (outcome_type == "continuous") {
    if (d_model_type == "empirical") {
      return(-1)
    } else if (d_model_type == "linear-over-t") {
      fit <- function() {
        return(-1)
      }
      class(fit) <- "dpart1cts"
    ...
    ### 2. Estimate E[phi_{it}^{otimes 2} | S_it]
    if (d_model_type == "linear-over-t") {
      predictor <- cbind(dta_train[, moderator], rep(1:total_T, times = sample_size), phi)
      pred_df <- as.data.frame(predictor)
      colnames(pred_df) <- c(moderator, "t", "phi")
      fit_formula <- as.formula(paste0("phi~", paste0(moderator, collapse = "+"), "+t"))
      fit <- lm(fit_formula, data = pred_df)
      # predictor <- exp(rep(1:total_T, times = sample_size))
      # fit <- lm(phi ~ predictor)
      # print(fit)
      return(fit)
    } else if (d_model_type == "empirical") {
      t_specific_mean <- matrix(NA, nrow = total_T, ncol = 1)
      for (t in 1:total_T) {
        it_idx <- seq(from = t, by = total_T, length = sample_size)
        t_specific_mean[t] <- mean(phi[it_idx])
      }
      return(t_specific_mean)
      # return(phi)
      # return(mean(phi))
    } else if (d_model_type == "exp-over-t") {
      predictor <- cbind(dta_train[, moderator], exp(rep(1:total_T, times = sample_size)), phi)
      pred_df <- as.data.frame(predictor)
      colnames(pred_df) <- c(moderator, "t", "phi")
      fit_formula <- as.formula(paste0("phi~", paste0(moderator, collapse = "+"), "+t"))
      fit <- lm(fit_formula, data = pred_df)
      ...
      predictor <- cbind(dta_train[, moderator], exp(rep(1:total_T, times = sample_size)), rep(1:total_T, times = sample_size), phi)
      pred_df <- as.data.frame(predictor)
      colnames(pred_df) <- c(moderator, "exp_t", "t", "phi")
      fit_formula <- as.formula(paste0("phi~", paste0(moderator, collapse = "+"), "+exp_t+t"))
      fit <- earth::earth(fit_formula, data = pred_df)
      return(fit)
    } else if (d_model_type == "gam") {
      predictor <- cbind(dta_train[, moderator], rep(1:total_T, times = sample_size), phi)
      ...
      colnames(pred_df) <- c(moderator, "t", "phi")
      if (is.null(moderator)) {
      fit_formula <- as.formula(paste0("phi~", "+s(t)"))
      } else {
        fit_formula <- as.formula(paste0("phi~", paste0("s(", moderator, ")",collapse = "+"), "+s(t)"))
      }
      fit <- mgcv::gam(fit_formula, data = pred_df)
      return(fit)
    }
