...
wcls_original <- function(dta,
                          ...
                          availability = NULL,
                          numerator_prob = 0.5,
                          no_se = FALSE # if TRUE, skip the sandwich/small-sample-correction (mostly for simulations)
) {
    ...
    ##### Early exit if SE not requested (for simulations) #####
    if (no_se) {
        Xnames <- c("Intercept", moderator)
        Znames <- c("Intercept", control_var)
        names(beta_hat)  <- Xnames
        names(alpha_hat) <- Znames
