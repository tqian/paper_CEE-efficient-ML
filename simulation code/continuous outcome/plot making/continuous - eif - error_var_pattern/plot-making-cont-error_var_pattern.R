...
result <- readRDS("../../simu-error_var_pattern/result_all.RDS")
...
result_cp <- result_cp %>% filter(sample_size == 50,
                                  control_pattern_tuning_param <= 3,
...
result_cp <- result_cp %>% filter(sample_size <= 50, control_pattern_tuning_param <= 3)
...
result_re <- result_sd_nowcls %>%
  left_join(result_sd_wcls,
            by = c("S_pattern", "rand_prob_pattern", "rand_prob_tuning_param", 
                   "control_pattern", "control_pattern_tuning_param", 
                   "error_var_pattern", "error_var_pattern_tuning_param",
                   "error_var_scale_tuning_param", "error_corr_tuning_param",
...
methods_to_plot <- c("eif_oracle..", "eif.gam.", "eif.gam.cf",
                     "eif.ranger.", "eif.ranger.cf",
                     "eif.sl.all.", "eif.sl.all.cf")
result_re <- result_re %>% filter(sample_size == 50,
                                  control_pattern_tuning_param <= 3,
                                  method_label %in% methods_to_plot)
...
plot_mse <- result_mse %>%
    filter(moderator == "NULL", error_var_pattern_tuning_param == 1) %>%
    ...
    # ggtitle(TeX(r'(Mean Squared Error (Under $\lambda_2 = 1$))')) +
    theme_bw() + 
    mytheme +
...
plot_cp <- result_cp %>%
    ...
    ggplot(aes(x = error_var_pattern_tuning_param, y = beta0_cp, 
               color = method_plot,
               linetype = cross_fit, linewidth = cross_fit)) +
    geom_line() +
    scale_linewidth_manual(name = "Cross-fitting", values = c(0.5, 1)) +
    ...
    xlab(TeX(r'($\lambda_2$ (nonstationarity in variance))')) + 
    # ylab("CP") +
...
plot_re <- result_re %>%
    filter(moderator == "NULL") %>%
    ggplot(aes(x = error_var_pattern_tuning_param, y = beta0_re, 
               color = method_plot,
               linetype = cross_fit, linewidth = cross_fit)) +
    # geom_jitter() + 
    geom_line() +
...
pdf(file = "../../simulation-eif-cont-error_var_pattern.pdf", width = 15/1.2, height = 8.5/1.2)
