# Data-generating model for Section 5.5 of the EJS revision: behavior under a
# large-T regime with constant CEE in t.
#
# Differs from sim-5.4's dgm_cont_time_varying_CEE.R only in the CEE structure:
#   5.4:  gamma_t(beta) = beta_0 + beta_1 * (t / T)   (time-moderated)
#   5.5:  gamma_t(beta) = beta_0                       (constant in t)
# S_t ~ Unif[-2, 2] is retained as a covariate driving mu_t(H_t, 0) via the same
# 4 control_patterns as the existing simulations.
...
# Constant CEE: gamma_t = beta_0 = 0.5.
const_beta0 <- 0.5
...
dgm_cont_const_CEE <- function(sample_size,
                               total_T,
    ...
    for (t in 1:total_T) {
        ...
        ## Constant-in-t CEE:  gamma_t = beta_0
        expect_Y_A1 <- expect_Y_A0 + const_beta0
