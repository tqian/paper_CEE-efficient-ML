...
methods_to_plot <- c("emee..", "eif_oracle..", "eif.gam.", "eif.gam.cf",
                     "eif.ranger.", "eif.ranger.cf",
                     "eif.stack.", "eif.stack.cf")
...
result_cp$method_plot[result_cp$estimator == "emee"] <- "emee"
...
result_mse$method_plot[result_mse$estimator == "emee"] <- "emee"
...
result_sd_nowcls <- result_sd %>% filter(estimator != "emee")
result_sd_wcls <- result_sd %>% 
  filter(estimator == "emee") %>%
  rename(beta0_sd_wcls = beta0_sd, beta1_sd_wcls = beta1_sd) %>%
  dplyr::select(!c(estimator, ml_method, cross_fit))
...
plot_cp <- result_cp %>%
    ...
    xlab(TeX(r'($\lambda$ (nonlinearity in $\mu_t$))')) + 
    # ylab("CP") +
...
pdf(file = "simulation-eif-count-control_pattern.pdf", width = 15/1.2, height = 8.5/1.2)
