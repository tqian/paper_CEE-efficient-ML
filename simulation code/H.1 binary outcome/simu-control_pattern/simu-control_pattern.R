...
SD1 <- expand.grid(S_pattern = c("constant"),
                   ...
                   control = c("dp"),
                   estimator = c("eif"),
                   ml_method = c("gam", "ranger", "sl.all"),
                   cross_fit = c(TRUE, FALSE),
                   total_T = 10,
                   stringsAsFactors = FALSE)

SD2 <- expand.grid(S_pattern = c("constant"),
                   rand_prob_pattern = c("constant"),
                   rand_prob_tuning_param = 1,
                   control_pattern = c("linear", "sine", "dbeta", "step"),
                   control_tuning_param = seq(from = 0, to = 1, by = 0.2),
...
SD_n30 <- rbind(SD_fast, SD_slower)
...
SD_n50100 <- rbind(SD_fast, SD_slower)
...
if (jobid == nrow(SD)) {
  ...
  for (jobid in 1:nrow(SD)) {
    ...
    # result_df <- result_df %>% mutate(cp = beta0_lci < 0.5 & 0.5 < beta0_uci)
    # mean(result_df$cp)
