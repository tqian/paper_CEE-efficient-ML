...
dgm_binary_time_s_tvvar <- function(sample_size, total_T,
  ...
  for (t in 1:total_T) {
    ...
    if (S_pattern == "constant") {
      ...
      prob_S <- rep(const_S_prob, sample_size)
      S <- rbinom(sample_size, 1, prob = prob_S)
      S <- ifelse(S == 0, -1, S)
    ...
    if (control_pattern == "linear") {
      ...
      expect_Y_A0 <- 
        exp(const_alpha0 + 
              const_alpha1 * dp / total_T + 
              const_alpha2 * S +
              ar1_tuning_param * Y_lag1)
    } else if (control_pattern == "sine") {
      expect_Y_A0 <- 
        exp(const_alpha0 + 
              control_tuning_param * sin(dp) + 
              control_tuning_param * sin(S)  +
              ar1_tuning_param * Y_lag1)
    } else if (control_pattern == "dbeta") {
      # S_transformed_within_unit_interval <- (S + const_S_halfrange + 1) / (2 * const_S_halfrange + 2)
      expect_Y_A0 <- 
        exp(const_alpha0 + 
              control_tuning_param * dbeta(dp / total_T, shape1 = 2, shape2 = 2) / 1.5 + 
              control_tuning_param * dbeta((S + 1) / 3, shape1 = 2, shape2 = 2) / 1.5 +
              ar1_tuning_param * Y_lag1)
    } else if (control_pattern == "step") {
      expect_Y_A0 <- 
        exp(const_alpha0 + 
              control_tuning_param * as.numeric(dp %% 2 == 0) +
              control_tuning_param * as.numeric(S == -1) +
              ar1_tuning_param * Y_lag1)
    }
    
    ## generate Y
    # expect_Y_A1 <- expect_Y_A0  + const_beta0 + const_beta1 * S
    expect_Y_A1 <- expect_Y_A0 * exp(const_beta0 + const_beta1 * S)
    expect_Y <- ifelse(A, expect_Y_A1, expect_Y_A0)
    Y <- rbinom(sample_size, 1, expect_Y)
