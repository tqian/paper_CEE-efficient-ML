...
plot_mse <- result_mse %>%
    filter(moderator == "NULL", control_pattern_tuning_param == 1) %>%
...
plot_cp <- result_cp %>%
    ...
    ggplot(aes(x = control_pattern_tuning_param, y = beta0_cp, 
               color = method_plot,
               linetype = cross_fit, linewidth = cross_fit)) +
    geom_line() +
    scale_linewidth_manual(name = "Cross-fitting", values = c(0.5, 1)) +
...
plot_re <- result_re %>%
    filter(moderator == "NULL") %>%
    ggplot(aes(x = control_pattern_tuning_param, y = beta0_re, 
               color = method_plot,
               linetype = cross_fit, linewidth = cross_fit)) +
    # geom_jitter() + 
    geom_line() +
...
pdf(file = "simulation-eif-cont-control_pattern.pdf", width = 15/1.2, height = 8.5/1.2)
